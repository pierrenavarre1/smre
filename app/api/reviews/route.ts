...
export async function GET(){
  ...
  if(!key) return NextResponse.json({reviews:[],error:'Google Places API key is not configured.'},{status:200});
  ...
  try{
    ...
    return NextResponse.json({rating:data.rating||0,userRatingCount:data.userRatingCount||0,googleMapsUri:data.googleMapsUri||'',reviews});
  }catch{
