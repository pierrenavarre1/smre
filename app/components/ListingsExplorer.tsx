...
export function ListingsExplorer({ items }: { items: RESOProperty[] }) {
  const [filters, setFilters] = useState<Filters>({ type: '', source: '', status: 'Active', beds: '', max: '', city: '' });
  ...
  return (
    <>
      <div className="filters">
        ...
        <select aria-label="Bedrooms" value={filters.beds} onChange={(e) => update('beds', e.target.value)}><option value="">Any beds</option><option value="2">2+ beds</option><option value="3">3+ beds</option><option value="4">4+ beds</option></select>
        <select aria-label="Maximum price" value={filters.max} onChange={(e) => update('max', e.target.value)}><option value="">Any price</option><option value="300000">Under $300k</option><option value="400000">Under $400k</option><option value="500000">Under $500k</option></select>
